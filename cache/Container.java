...
import com.userv.util.Compressions;
...
public final class Container {
	...
	public static Container decode(ByteBuffer buffer) throws IOException {
		...
		if (type == COMPRESSION_NONE) {
			...
			if (type == COMPRESSION_BZIP2) {
				uncompressed = Compressions.bunzip2(compressed);
			} else if (type == COMPRESSION_GZIP) {
				uncompressed = Compressions.gunzip(compressed);
			} else {
	...
	public ByteBuffer encode() throws IOException {
		...
		if (type == COMPRESSION_NONE) {
			...
			compressed = Compressions.gzip(bytes);
		} else if (type == COMPRESSION_BZIP2) {
			compressed = Compressions.bzip2(bytes);
		} else {
			throw new IOException("Invalid compression type");
		}
