...
import com.userv.util.Buffers;
import com.userv.util.Whirlpool;


/**
 * The {@link Cache} class provides a unified, high-level API for modifying the cache of a
 * Jagex game.
 * 
 * @author Graham
 * @author `Discardedx2
...
public final class Cache implements Closeable {
	...
	public ChecksumTable createChecksumTable() throws IOException {
		...
		for (int i = 0; i < size; i++) {
			...
			/*
			 * if there is actually a reference table, calculate the CRC, version and whirlpool
			 * hash
			 */
			if (buf.limit() > 0) { // some indices are not used, is this appropriate?
				...
				crc = Buffers.getCrcChecksum(buf);
				version = ref.getVersion();
				buf.position(0);
				whirlpool = Buffers.getWhirlpoolDigest(buf);
			}
	...
	/**
	 * Writes a file to the cache and updates the {@link ReferenceTable} that it is associated
	 * with.
	 * 
	 * @param type
	 *            The type of file.
	...
	public void write(int type, int file, Container container) throws IOException {
		...
		byte[] bytes = new byte[buffer.limit() - 2]; // last two bytes are the version and
